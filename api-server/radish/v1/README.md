...
**注意**: `config.php`は`radish/`直下に配置し、`v1/`フォルダ内のAPIファイルは`require_once __DIR__ . '/../config.php';`で読み込みます。
...
**重要**: `config.php`は`radish/`直下に配置してください。`v1/`フォルダ内のAPIファイルは親ディレクトリの`config.php`を読み込みます。
