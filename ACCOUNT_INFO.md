...
**サービス全体を管理する管理者アカウントです。弊社向け管理画面（`/company/login`）でログインしてください。**
...
- **管理画面**: `/company/dashboard`（弊社向け管理画面）
- **アクセス権限**: 全店舗へのアクセス権限あり（8店舗すべてを管理可能）

**ログイン方法**:
1. トップページから「弊社向け管理ページ」をクリック
2. または `/company/login` に直接アクセス
3. 上記のアカウント情報でログイン
4. 自動的に `/company/dashboard` にリダイレクトされます

**注意**: このアカウントは全店舗を管理できるため、店舗スタッフ用のログイン画面（`/staff/login`）でもログインできますが、弊社向け管理画面（`/company/login`）を使用することを推奨します。
...
2. または、直接 `/admin/users/password` にアクセス
...
**アクセス方法**:
1. `/admin/users` のスタッフ一覧から、対象ユーザーの「パスワード変更」ボタンをクリック
2. または、直接 `/admin/users/{ユーザーID}/password` にアクセス
