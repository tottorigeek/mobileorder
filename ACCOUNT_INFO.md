...
**サービス全体を管理する管理者アカウントです。運営者向け管理画面（`/company/login`）でログインしてください。**
...
- **管理画面**: `/company/dashboard`（運営者向け管理画面）
...
**注意**: このアカウントは全店舗を管理できるため、店舗スタッフ用のログイン画面（`/staff/login`）でもログインできますが、運営者向け管理画面（`/company/login`）を使用することを推奨します。
